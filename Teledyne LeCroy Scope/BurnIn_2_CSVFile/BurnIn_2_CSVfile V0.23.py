...
# -- Version 0.22 - Put Threshold detection by default on 40 - because we run BI1 for 23h
# --              - introduce a timer to stop the app after xx h of running  -in our case will be 23h
...
led_scope_1 = None
led_scope_2 = None
auto_stop_duration = 23  # Default to 23 hours for Burn In 1 duration - this can be change to 2h for Burn in 2 Pre-screening
...
# Function to set the timer and stop logging after the specified duration
def auto_stop_timer(duration_entry):
    global auto_stop_duration

    try:
        # Get the user-specified duration
        auto_stop_duration = int(duration_entry.get())
    except ValueError:
        messagebox.showerror("Error", "Please enter a valid integer for the duration.")
        return

    # Convert hours to seconds
    duration_seconds = auto_stop_duration * 3600

    def countdown_timer():
        nonlocal duration_seconds
        while duration_seconds > 0 and running_event.is_set():
            time.sleep(1)
            duration_seconds -= 1
        if running_event.is_set():
            stop_logging()

    # Start a thread for the countdown timer
...
def setup_gui():
    ...
    # Timer duration input field with label
    duration_label = tk.Label(root, text="Auto-Stop Timer (hours):", font=("Arial", 12))
    duration_label.pack()
    duration_entry = tk.Entry(root, font=("Arial", 12))
    duration_entry.insert(0, "23")  # Default value
    ...
    version_label = tk.Label(root, text="V0.23 by AdrianO", font=("Arial", 10), anchor="se")
