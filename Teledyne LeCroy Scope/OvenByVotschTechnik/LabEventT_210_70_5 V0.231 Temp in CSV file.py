    ...
    with open(filename, "w", newline="") as file:
        ...
        try:
            while True:
                ...
                cleaned_response = temp_response[2:].strip() if temp_response else ""

                # Simulate the same temperature reading every time for testing
                #cleaned_response = "100.0"
                ...
                if cleaned_response:
                    ...
                    # Convert to float for strict comparison
                    try:
                        temp_value = float(cleaned_response)
                    except ValueError:
                        temp_value = cleaned_response  # fallback if not a number

                    # Check for repeated values
                    last_temps.append(temp_value)
                    if len(last_temps) > 8:
                        last_temps.pop(0)
                    if len(last_temps) == 8 and all(t == last_temps[0] for t in last_temps):
